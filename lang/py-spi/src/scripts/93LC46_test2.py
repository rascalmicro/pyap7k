...
mask_read = [0x3, 0x0] # 7 or 6 address bits follow, 8 or 16 data bits return
mask_write = [0x2, 0x80] # 7 or 6 address bits follow, 8 or 16 data bits follow
...
#	cmd.append( mask_read )
#	cmd.append( addr << 1 )
	print "read: sending command", bin(cmd[0]), bin(cmd[1])
	result = spidev.msg([cmd[0],cmd[1],0],0)
	# todo: parse result
	return result
...
#		result = spidev.msg2([cmd[0], cmd[1], cmd[2],0,0,1], 0)
	...
	cmd.append(mask_write[0])
	cmd.append(mask_write[1] | addr)
	cmd.append( value )
	print "write command:", bin(cmd[0]), bin(cmd[1]), bin(cmd[2])
	result = spidev.msg([cmd[0], cmd[1], cmd[2],0,0,1], 0)
